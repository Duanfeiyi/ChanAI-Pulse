classdef ChannelSimulatorV3App < handle
    ...
    properties (Access = private)
        ...
        HelpFloatingButton matlab.ui.control.Button
        HelpPanel matlab.ui.container.Panel
        HelpHintLabel matlab.ui.control.Label
        HelpQuestionList matlab.ui.control.ListBox
        HelpAnswerArea matlab.ui.control.TextArea
        HelpCloseButton matlab.ui.control.Button
        HelpIsOpen logical = false
    end
    ...
    methods (Access = private)
        function createComponents(app)
            ...
            app.createHelpAssistant(blue, muted);
        ...
        function layoutApplication(app)
            ...
            app.layoutHelpAssistant();
        end

        function createHelpAssistant(app, blue, muted)
            % A floating, always-on-top help bubble that answers the most
            % common operational questions.  Like the progress card it is a
            % direct UIFigure child, so it floats above the tab content.
            app.HelpFloatingButton = uibutton(app.UIFigure, "push", ...
                "Text", "?", "FontSize", 22, "FontWeight", "bold", ...
                "FontColor", "white", "BackgroundColor", blue, ...
                "Tooltip", "操作帮助", ...
                "ButtonPushedFcn", @(~, ~) app.toggleHelpAssistant());

            app.HelpPanel = uipanel(app.UIFigure, ...
                "Title", "❓ 操作帮助", "FontWeight", "bold", ...
                "FontSize", 14, "BackgroundColor", "white", ...
                "BorderColor", blue, "Visible", "off");
            grid = uigridlayout(app.HelpPanel, [4, 1], ...
                "RowHeight", {24, 200, "1x", 34}, ...
                "Padding", [12, 10, 12, 10], "RowSpacing", 8);
            app.HelpHintLabel = uilabel(grid, ...
                "Text", "点击下方问题查看操作说明", ...
                "FontColor", muted, "FontWeight", "bold");
            app.HelpQuestionList = uilistbox(grid, ...
                "ValueChangedFcn", @(~, ~) app.refreshHelpAnswer());
            app.HelpAnswerArea = uitextarea(grid, "Editable", "off", ...
                "WordWrap", "on", "FontSize", 12, ...
                "BackgroundColor", [0.97, 0.98, 1]);
            app.HelpCloseButton = uibutton(grid, "push", ...
                "Text", "✕ 收起", ...
                "ButtonPushedFcn", @(~, ~) app.toggleHelpAssistant());
            app.refreshHelpQuestions();
            app.refreshHelpAnswer();
        end

        function layoutHelpAssistant(app)
            % Keep the floating help bubble and its panel inside the window
            % as it resizes, anchored to the bottom-right corner.
            if isempty(app.UIFigure) || ~isvalid(app.UIFigure) || ...
                    isempty(app.HelpFloatingButton) || ...
                    ~isvalid(app.HelpFloatingButton)
                return;
            end
            figurePosition = app.UIFigure.Position;
            figureWidth = figurePosition(3);
            ...
            margin = 24;
            buttonSize = 54;
            app.HelpFloatingButton.Position = ...
                [figureWidth - margin - buttonSize, margin, buttonSize, buttonSize];
            panelWidth = min(400, max(300, figureWidth - 2 * margin - 60));
            panelHeight = min(500, max(300, figureHeight - 2 * margin - buttonSize - 12));
            app.HelpPanel.Position = [ ...
                figureWidth - margin - panelWidth, ...
                margin + buttonSize + 12, ...
                panelWidth, panelHeight];
        end

        function toggleHelpAssistant(app)
            app.HelpIsOpen = ~app.HelpIsOpen;
            if app.HelpIsOpen
                try
                    uistack(app.HelpPanel, "top");
                    uistack(app.HelpFloatingButton, "top");
                catch
                end
                app.HelpPanel.Visible = "on";
                app.HelpFloatingButton.Text = "✕";
            else
        ...
        function refreshHelpQuestions(app)
            entries = app.helpEntries();
            if app.CurrentLanguage == "en"
                questions = string({entries.q_en});
            else
                questions = string({entries.q_zh});
            end
            app.HelpQuestionList.Items = questions;
            app.HelpQuestionList.ItemsData = (1:numel(questions)).';
            current = app.HelpQuestionList.Value;
            if isempty(current) || ~any(app.HelpQuestionList.ItemsData == current)
                app.HelpQuestionList.Value = 1;
            end
        end

        function refreshHelpAnswer(app)
            entries = app.helpEntries();
            index = double(app.HelpQuestionList.Value);
            if isempty(index) || ~isscalar(index) || index < 1 || index > numel(entries)
                index = 1;
            end
            if app.CurrentLanguage == "en"
                app.HelpAnswerArea.Value = entries(index).a_en;
            else
                app.HelpAnswerArea.Value = entries(index).a_zh;
            end
        end

        function updateHelpAssistantLanguage(app)
            if isempty(app.HelpPanel) || ~isvalid(app.HelpPanel)
                return;
            end
            if app.CurrentLanguage == "en"
                app.HelpPanel.Title = "❓ Operation Help";
                app.HelpHintLabel.Text = "Select a question below to see the answer";
                app.HelpFloatingButton.Tooltip = "Operation help";
                app.HelpCloseButton.Text = "✕ Close";
            else
                app.HelpPanel.Title = "❓ 操作帮助";
                app.HelpHintLabel.Text = "点击下方问题查看操作说明";
                app.HelpFloatingButton.Tooltip = "操作帮助";
                app.HelpCloseButton.Text = "✕ 收起";
            end
            app.refreshHelpQuestions();
            app.refreshHelpAnswer();
        end

        function entries = helpEntries(~)
            qZh = { ...
                "如何导入信道数据文件？"; ...
                "如何选择任务类型（外推 / 内插）？"; ...
                "“自动 80/20” 与“精确手动区间”有何区别？"; ...
                "如何手动设置已知区与目标区？"; ...
                "开始预测前需要完成哪些步骤？"; ...
                "普通模式与高级模式有何区别？"; ...
                "如何导出 CIR / CTF / Manifest？"; ...
                "为什么需要配置 Python / PyTorch 环境？"; ...
                "页面上的 “WARNING” 是什么意思？"; ...
                "如何验证预测结果的准确度？" ...
                };
            qEn = { ...
                "How do I import a channel data file?"; ...
                "How do I choose the task type (extrapolation / interpolation)?"; ...
                "What is the difference between “Automatic 80/20” and “Exact manual ranges”?"; ...
                "How do I set the known and target regions manually?"; ...
                "What steps must I complete before starting a prediction?"; ...
                "What is the difference between Standard and Advanced mode?"; ...
                "How do I export CIR / CTF / Manifest?"; ...
                "Why do I need to configure a Python / PyTorch environment?"; ...
                "What does “WARNING” mean on the page?"; ...
                "How do I verify prediction accuracy?" ...
                };
            aZh = { ...
                "点击模块一左上角“浏览信道文件…”，选择 v3 信道 HDF5（.h5 / .hdf5）或 .mat 文件。选择 .mat 会自动打开 MAT 数据转换向导，转换完成后自动加载；也可点击“MAT 数据转换向导…”手动转换。选定文件后点击“加载、验证并分析”。"; ...
                "在模块一“任务类型”下拉框选择。“外推：已知 → 未来”用左侧已知区预测右侧未来目标；“内插：两侧 → 中间”用两侧已知区预测中间缺口。"; ...
                "“自动 80/20”由数据长度自动划分约 80% 已知、20% 目标，适合快速体验；“精确手动区间”需在下方手动填写已知区与目标区，适合精确控制。"; ...
                "把任务预设改为“精确手动区间”，在“已知区”和“目标区”输入形如 0:30,40:60 的区间。输入依据默认“原始样本编号（推荐）”；选“MATLAB 数组位置（高级）”则使用从 1 开始的内部索引。"; ...
                "① 选择并加载信道文件；② 设置任务类型与区间；③ 点击“加载、验证并分析”等待 PASS；④ 点击“开始预测”。模块二会自动标定，模块三会自动生成预测 CIR。"; ...
                "普通模式自动选择优化器、生成后端与预测模型，隐藏高级面板；高级模式可手动选择生成后端、优化方法、模型、适配与设备。手动选择不会被系统静默替换。"; ...
                "预测成功生成后，在“3. 预测结果”页点击“导出 CIR / CTF / Manifest”，选择目录即可导出 predicted_cir.h5、predicted_ctf.h5、prediction_manifest.json 等文件。"; ...
                "模块三的神经网络等预测模型依赖 Python / PyTorch。可用环境变量 CHANAI_STEP10_PYTHON 指定解释器。未配置时仍可导入和分析数据，但预测生成会明确停止。"; ...
                "WARNING 表示回测性能或连续性等超过参考值，只是警告、不会阻止生成，CIR 仍会生成并可导出；只有硬性技术错误才会拒绝运行。"; ...
                "预测页面不显示准确度。请使用独立程序 ChannelBenchmark，加载完整原始标准 HDF5 与预测导出目录，得到客观的 NMSE / 相关性等对比。" ...
                };
            aEn = { ...
                "Click “Browse channel file…” in the top-left of Module 1 and choose a v3 channel HDF5 (.h5 / .hdf5) or a .mat file. Choosing .mat opens the MAT conversion wizard, which auto-loads on completion; you can also open it manually. After selecting a file, click “Load, validate & analyze”."; ...
                "Use the “Task type” dropdown in Module 1. “Extrapolation: known → future” predicts future targets from the left known region; “Interpolation: sides → middle” predicts the middle gap from both known sides."; ...
                "“Automatic 80/20” splits about 80% known / 20% target from the data length and suits a quick trial; “Exact manual ranges” lets you type the known and target regions below for precise control."; ...
                "Switch the preset to “Exact manual ranges” and enter ranges such as 0:30,40:60 in the known and target fields. The basis defaults to “Original sample ID (recommended)”; choose “MATLAB array index (advanced)” for 1-based internal indices."; ...
                "① select and load a channel file; ② set the task type and ranges; ③ click “Load, validate & analyze” and wait for PASS; ④ click “Start prediction”. Module 2 calibrates automatically and Module 3 generates the predicted CIR."; ...
                "Standard mode automatically chooses the optimizer, generator backend, and prediction model and hides the advanced panel; Advanced mode lets you manually pick the backend, optimizer, model, adaptation, and device. Manual choices are never silently replaced."; ...
                "After a successful prediction, click “Export CIR / CTF / Manifest” on the “Prediction Result” page and choose a directory to export predicted_cir.h5, predicted_ctf.h5, prediction_manifest.json, and more."; ...
                "The neural and other prediction models in Module 3 require Python / PyTorch. Set the interpreter with the CHANAI_STEP10_PYTHON environment variable. Without it you can still import and analyze data, but prediction will stop explicitly."; ...
                "WARNING means backtest performance or continuity exceeds a reference value; it only warns and never blocks generation — the CIR is still produced and exportable. Only hard technical errors are rejected."; ...
                "The prediction page does not show accuracy. Use the separate ChannelBenchmark program with the complete original standard HDF5 and the prediction export directory to obtain objective NMSE / correlation comparisons." ...
                };
            entries = struct( ...
                "q_zh", qZh, "q_en", qEn, "a_zh", aZh, "a_en", aEn);
        end
        ...
        function switchLanguage(app)
            ...
            app.localizeCurrentView();
            app.updateHelpAssistantLanguage();
        end
