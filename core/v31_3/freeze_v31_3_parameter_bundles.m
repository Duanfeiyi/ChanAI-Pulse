...
if any(~isfinite(scores)) || any(scores < 0)
    error("freeze_v31_3_parameter_bundles:InvalidScores", ...
        "Sensitivity scores must be finite and nonnegative.");
end
scores(scores < config.freezing.sensitivity_score_floor) = 0;
if sum(scores) <= 0
    error("freeze_v31_3_parameter_bundles:InvalidScores", ...
        "Sensitivity scores do not contain evidence above the configured floor.");
end
...
frozen = struct( ...
    ...
    "test_truth_used_for_selection", ...
        string(config.selection_partition) ~= "validation", ...
    "parameter_names", config.parameter_names, ...
    "parameter_bounds", config.parameter_bounds, ...
    "sensitivity_scores", table2struct(sensitivitySummary), ...
    "effective_sensitivity_scores", scores, ...
    "decisions", decisions, ...
