...
%   Route speed and antenna counts are catalog metadata only: they do not
%   alter the parameter labels or become predictor inputs in this corpus.
%   Configured Full CIR generation remains a later benchmark activity.
...
catalog.route_configuration_role = repmat( ...
    string(config.route.configuration_role), groupCount, 1);
...
corpus.sequence.provenance.v31_2 = struct( ...
    ...
    "measured_data", false, ...
    "full_cir_generated", false, ...
    "route_speed_applied_to_labels", false, ...
    "antenna_counts_applied_to_labels", false, ...
    "route_configuration_role", string(config.route.configuration_role), ...
    "core_modified", false);
...
corpus.summary.route_configuration_role = string(config.route.configuration_role);
end
...
if ~isfield(config.route, "configuration_role") || ...
        string(config.route.configuration_role) ~= "catalog_metadata_only"
    error("create_v31_2_training_corpus:InvalidConfig", ...
        "v3.1-2 route speed and antenna counts must be declared as catalog metadata only.");
end
