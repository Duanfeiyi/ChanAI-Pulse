function Opti_FCF(app, cm, tx_track, rx_track)
    loop = 50;
    ...
    %当前（最小）误差
    Min_MSE = tools.cal_MSE(app.MeasureData.x,app.MeasureData.y,deltaF,abs(mean_fcf));
    %显示当前参数
    ...
    for Opti_loop = 1:Opti_loop_num   %优化循环
        ...
        for i_loop = 1:loop  % 计算FCF
            ...
            H_CTF = mf.H2ctf(H, freq_sample, B, delay);
        ...
        %计算误差
        error_MSE(Opti_loop) = tools.cal_MSE(app.MeasureData.x,app.MeasureData.y,deltaF,abs(mean_fcf));
        plot(app.Figure_Fitting,deltaF, abs(mean_fcf), '--','Color',[0.5 0.5 0.5],'Linewidth',0.5);
        ...
        if error_MSE(Opti_loop) < Min_MSE
            ...
            app.Line_BestData = plot(app.Figure_Fitting,deltaF,abs(mean_fcf),'r-','LineWidth',1.2);
            app.Figure_Fitting.NextPlot = "add";
            legend(app.Figure_Fitting,[app.Line_Fitting_Meas,app.Line_InitialData,app.Line_BestData],'Measurement data','Initial configuration','Configuration after optimization');
            app.settings.Value = ''; 
