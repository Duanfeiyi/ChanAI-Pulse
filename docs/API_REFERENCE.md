...
| `select_generator_backend(requestedBackend, dimensions, options)`<br>`core/generation/select_generator_backend.m` | Resolves `auto` or an explicit production backend before execution. It checks Tx/Rx/Nt compatibility and availability, excludes Mock, records every candidate, and never changes the requested dimensions. | Step 12 ordinary/advanced UI and selection regression tests. |
| `run_full_6gpcm_public_api_adapter(config, hooks)`<br>`core/generation/run_full_6gpcm_public_api_adapter.m` | Wraps the read-only external Full 6GPCM public classes with configurable Tx/Rx/Nt, exact dimension checks, per-sample seeds and before/after tree hashes. | Step 12 MIMO/static/dynamic formal generation and external integration test. |
| `collapse_cir_to_narrowband(dataset)`<br>`core/generation/collapse_cir_to_narrowband.m` | Converts resolved multipath CIR into one unresolved complex baseband tap while preserving Tx/Rx/Nt/N_sample and recording the collapse rule. | Narrowband 1-plot product path. |
| `create_capability_default_calibration(analysis, config)`<br>`core/optimization/create_capability_default_calibration.m` | Returns an auditable successful narrowband calibration using versioned defaults when a one-tap input has no identifiable PDP for Grid/SA. | Step 12 narrowband Module 2 path. |
