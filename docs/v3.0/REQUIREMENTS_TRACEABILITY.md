...
| V3-REQ-012 | 预测器输出可供 6GPCM 使用的目标参数 | 模块三 | 9、10 | Step 9 已固定全部8参数契约、标准 `DS_mu/KF_mu`、单位/范围/来源/HDF5；实际模型输出待Step 10 | 进行中 |
| V3-REQ-013 | 预测参数必须通过 6GPCM 转换为预测 CIR | 模块三 | 11 | 参数到 CIR 端到端测试 | 待实现 |
| V3-REQ-014 | MATLAB 或 Python 均可实现算法 | 全部 | 1、6、10 | 双语言最小接口示例 | 已冻结 |
| V3-REQ-015 | 所有正式函数输入输出清晰并有接口文档 | 全部 | 1、6、9、10、14 | 既有接口指南及 `STEP_9_INTERFACE_GUIDE.md`；Step 9 MATLAB/Python HDF5 交叉读取已通过 | 进行中 |
...
| V3-REQ-022 | 预测训练数据按路线/场景组切分并使用训练集专属归一化，禁止相邻滑窗泄漏 | 模块三 | 9、10 | `test_step9_predictor_data.m` 验证7/1/2组切分、组不重叠和验证/测试变化不影响Z-score | 进行中 |
| V3-REQ-023 | 泛用预训练与上传数据微调分层，支持 `pretrain/auto/off/force` 且阈值由实验确定 | 模块三、GUI | 9、10、12 | Step 9已记录模式与资格元数据；模型、阈值和正式UI待Step 10/12 | 进行中 |
